...
- **Document the legal-position contract for the unwinnability analyzers.** The analyzers — and the `BasicHelpmateExistenceTheorem` shortcut — are guaranteed only on strictly legal positions; on illegal input the result is undefined and may disagree between `unwinnableQuick` and `unwinnableFull` (and with chasolver). Legality is decidable only by infeasible retrograde / proof-game search, so we cannot enforce it — state the contract at the package level and on the public analyzers instead. Concrete out-of-domain examples found via the chasolver cross-check: the retro-illegal `8/8/8/8/2N5/8/k1K5/1B6 b` (KBNvK) and `8/8/8/8/8/B7/B7/k1K5 w` (KBBvK, opposite bishops), where `unwinnableFull` returns `WINNABLE` via the theorem while `unwinnableQuick` and chasolver return `UNWINNABLE`.
- **Add the known retro-illegal theorem-counterexamples to the tests** (the two above plus the ~3+15 white / 1 black representatives enumerated in basic-helpmate-existence) as documented out-of-domain regression cases, asserting ashlar's current behaviour and noting they are illegal / out-of-scope. **Open decision:** document + test only (recommended) vs hardcode per-position corrections so `unwinnableFull` returns the correct verdict on them too. Hardcode pros: finite/known set, makes the theorem total-correct on covered classes, removes the quick/full disagreement on these; cons: couples ashlar to bhe's per-class enumeration (re-sync whenever covered classes change — they just did in 1.2.0), gives a false sense of general illegal-position handling, and only fixes outputs that cannot arise in a legal game.
