...
- **`canClaimFiftyMoveRuleWithOwnMove()` follows the strict FIDE 9.3 reading — deliberate divergence from python-chess at one corner case.** FIDE 9.3 frames the 50-move claim as announced *before* the move is played; the 50 moves are about history, and the candidate move's outcome is incidental to whether the no-progress condition has been met. clean-chess returns `true` at halfmove clock ≥ 99 if any legal move is non-pawn and non-capture — even if that move would deliver checkmate. python-chess (per `1064bf59`, "Allow claiming fifty-moves before final move (fixes #632)") pushes the candidate and re-checks `is_fifty_moves` on the post-position under its "once checkmated, it is too late to claim" reading, so it rejects mate-in-one candidates. Both readings are defensible; clean-chess takes the strict FIDE reading because the player making the claim does so before the move and the rule is about history. The disagreement only manifests at constructed edge positions where the only non-zeroing legal move at clock 99 ends the game; no corpus fixture surfaces it in practice. Pinned in `TestBoardClaimWithOwnMove#canClaimFiftyMoveRuleWithOwnMoveTrueEvenWhenOnlyNonZeroingMoveIsMate`.
