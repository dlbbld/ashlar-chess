 ...
 * {@code static final Set<String> VALUES} at class load time by calling these helpers. The string format mirrors what
 * the (now-deleted) {@code Generate*SanValidateStrict} scripts emitted as enum constants - e.g. {@code "N1A2"}
 * (piece-letter + rank-disambig + target square) - so the {@code *Calculate} consumers parse the same shape.
 */
final class SanValidateStaticallyStrictHelpers {
  ...
  static void appendMoveWithFile(Set<String> set, Square toSquare, io.github.dlbbld.ashlarchess.board.enums.File file,
      PieceType pieceType) {
