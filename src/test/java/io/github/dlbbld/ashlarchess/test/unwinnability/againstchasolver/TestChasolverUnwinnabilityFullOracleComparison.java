...
class TestChasolverUnwinnabilityFullOracleComparison {
  ...
  @Test
  void chaPositionsMatchFullChasolverOracle() {
    ...
    for (final PgnTest pgnTest : PgnTest.values()) {
      ...
      for (final PgnFen testCase : testCaseList.list()) {
        ...
        check(testCase, Side.WHITE, ChasolverUnwinnabilityOracle.get(testCase.finalFen()).fullWhite(), failures,
            remainingAcceptedDifferenceSet);
        check(testCase, Side.BLACK, ChasolverUnwinnabilityOracle.get(testCase.finalFen()).fullBlack(), failures,
            remainingAcceptedDifferenceSet);

        if (checkedPositionCount % PROGRESS_LOG_INTERVAL == 0) {
          logger.info("Checked {} chasolver positions, failures so far: {}", checkedPositionCount, failures.size());
        }
      }
    }

    logger.info("Checked {} chasolver positions, failures: {}", checkedPositionCount, failures.size());
  ...
  private static Set<AcceptedDifference> readAcceptedDifferenceSet() {
    ...
    if (lines.isEmpty() || !"pgnName\tside\texpected\tactual\tfen\treason".equals(Nulls.get(lines, 0))) {
      throw new ProgrammingMistakeException("Unexpected full chasolver unwinnability accepted-differences header");
    }
    ...
    for (int i = 1; i < lines.size(); i++) {
      ...
      if (itemArray.length != 6) {
        throw new ProgrammingMistakeException("Invalid full chasolver unwinnability accepted-differences row: " + line);
      }
  ...
  private static String formatFailureMessage(int checkedPositionCount, List<String> failures) {
    ...
    return "Full chasolver unwinnability oracle mismatches for " + failures.size() + " of " + checkedPositionCount
        + " chasolver positions:\n" + Nulls.join("\n", printedFailures);
  }
