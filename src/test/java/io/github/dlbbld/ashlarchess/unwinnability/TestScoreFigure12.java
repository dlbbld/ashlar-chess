...
class TestScoreFigure12 {
  ...
  void winnerCaptureIsReward() {
    ...
    assertEquals(1, Score.increment(board, move(board, "e4", "e5"), Side.WHITE));
  }
  ...
  void winnerPawnPushIsReward() {
    ...
    assertEquals(1, Score.increment(board, move(board, "e2", "e3"), Side.WHITE));
    assertEquals(1, Score.increment(board, move(board, "e2", "e4"), Side.WHITE));
  }
  ...
  void winnerKingTowardCornerIsReward() {
    ...
    assertEquals(1, Score.increment(board, move(board, "d4", "c5"), Side.WHITE));
    assertEquals(0, Score.increment(board, move(board, "d4", "e3"), Side.WHITE));
  }
  ...
  void winnerPlainPieceMoveIsNormal() {
    ...
    assertEquals(0, Score.increment(board, move(board, "d4", "a4"), Side.WHITE));
  }
  ...
  void loserPromotionToQueenOrRookIsPunishUnderTheMaterialCondition() {
    ...
    assertEquals(-2, Score.increment(board, promotion(board, "g2", "g1", PromotionPieceType.QUEEN), Side.WHITE));
    assertEquals(-2, Score.increment(board, promotion(board, "g2", "g1", PromotionPieceType.ROOK), Side.WHITE));
    // Line 7: any other pawn move - including an underpromotion - is a Reward.
    assertEquals(1, Score.increment(board, promotion(board, "g2", "g1", PromotionPieceType.KNIGHT), Side.WHITE));
    assertEquals(1, Score.increment(board, promotion(board, "g2", "g1", PromotionPieceType.BISHOP), Side.WHITE));
  }
  ...
  void loserPawnMoveIsRewardUnderTheMaterialCondition() {
    ...
    assertEquals(1, Score.increment(board, move(board, "g4", "g3"), Side.WHITE));
  }
  ...
  void loserCaptureIsPunish() {
    ...
    assertEquals(-2, Score.increment(board, move(board, "d5", "e4"), Side.WHITE));
  }
  ...
  void loserKingTowardCornerIsReward() {
    ...
    assertEquals(1, Score.increment(board, move(board, "d7", "c8"), Side.WHITE));
    assertEquals(0, Score.increment(board, move(board, "d7", "e6"), Side.WHITE));
  }
  ...
  void castlingIsNormal() {
    ...
    for (final MoveSpecification candidate : board.getLegalMoveSpecifications()) {
      if (candidate.isCastling()) {
        assertEquals(0, Score.increment(board, candidate, Side.WHITE));
        return;
      }
    }
