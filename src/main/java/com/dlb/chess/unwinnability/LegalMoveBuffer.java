 ...
 * traversal order); deliberately not sorted — per the 12.1.0 move-order policy the search board's iteration order is
 * an internal performance choice. Read-only at callsites: the inherited {@link AbstractList} mutators
 * ({@code add}, {@code clear}, {@code remove}, …) throw {@link UnsupportedOperationException} by default; the
 * package-private {@link #append(LegalMove)} and {@link #reset()} below are the only mutators and are only callable
