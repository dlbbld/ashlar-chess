...
public final class BitboardLegalMoveFactory {
   ...
   * {@link BitboardPosition#legalMoves} and run through {@link #toLegalMove}; castling moves are computed inline
   * via {@link #addCastlingLegalMoves} using {@link CastlingUtility}'s bitboard overloads. Returns an
  ...
  public static ImmutableList<LegalMove> calculateLegalMoves(BitboardPosition bitboardPosition, Side havingMove,
    ...
    for (final MoveSpecification spec : bitboardPosition.legalMoves(havingMove, enPassantBit)) {
      result.add(toLegalMove(bitboardPosition, spec, havingMove));
    }
    addCastlingLegalMoves(result, bitboardPosition, havingMove, castlingRight);
    return Nulls.copyOfList(result);
  }

  /**
   * Adds the legal castling moves for {@code havingMove} (king-side and queen-side, in that order) to
   * {@code result}, using {@link CastlingUtility}'s bitboard-overload castling checks. Moved here from
   * {@code KingCastlingLegalMoves} so the production legal-move pipeline does not depend on the relocated
   * {@code com.dlb.chess.moves.*LegalMoves} subtree (which lives in {@code src/test/} as the differential-test
   * oracle).
   */
  private static void addCastlingLegalMoves(Set<LegalMove> result, BitboardPosition bitboardPosition, Side havingMove,
    ...
    if (havingMove == Side.WHITE) {
      if (CastlingUtility.calculateQueenSideCastlingCheck(bitboardPosition, havingMove,
        ...
        result.add(CastlingConstants.WHITE_QUEEN_SIDE_CASTLING_MOVE);
      }
      if (CastlingUtility.calculateKingSideCastlingCheck(bitboardPosition, havingMove,
          castlingRight) == CastlingCheck.SUCCESS) {
        result.add(CastlingConstants.WHITE_KING_SIDE_CASTLING_MOVE);
      }
    } else if (havingMove == Side.BLACK) {
      if (CastlingUtility.calculateQueenSideCastlingCheck(bitboardPosition, havingMove,
          castlingRight) == CastlingCheck.SUCCESS) {
        result.add(CastlingConstants.BLACK_QUEEN_SIDE_CASTLING_MOVE);
      }
      if (CastlingUtility.calculateKingSideCastlingCheck(bitboardPosition, havingMove,
          castlingRight) == CastlingCheck.SUCCESS) {
        result.add(CastlingConstants.BLACK_KING_SIDE_CASTLING_MOVE);
      }
    } else {
      throw new IllegalArgumentException("addCastlingLegalMoves requires Side.WHITE or Side.BLACK, got " + havingMove);
    }
  }
}
