...
/**
 * The library's flagship feature - unwinnability analysis. Decides whether a position is <em>unwinnable for a side</em>
 * - no legal sequence can end with that side giving checkmate, even if the opponent cooperates - and the symmetric
 * notion of a <em>dead position</em> (unwinnable for both sides).
 *
 * <p>
 * Since 22.0.0 the engine is ashlar's own independent, paper-derived implementation of Miguel Ambrona's FUN 2022 paper
 * <em>A Practical Algorithm for Chess Unwinnability</em> (Figures 5-13, Lemmas 5/6, Theorem 12), governed by the
 * committed specification {@code fun22-spec.pdf} and derived from the paper only. It replaces the earlier Java port of
 ...
 * - and most chess libraries get them wrong. This analysis handles those cases as rule questions rather than material
 * shortcuts; its definite verdicts are proof results, while hard full searches may still stop as {@code UNDETERMINED}.
 *
 * <h2>Two variants</h2>
 *
 * <ul>
 * <li><strong>Quick</strong> ({@link io.github.dlbbld.ashlarchess.unwinnability.UnwinnableQuickAnalyzer}) - the paper's
 * Figure 10: structural and bounded, three-valued: {@code UNWINNABLE}, {@code WINNABLE} (only on quickly matable
 * positions), or {@code POSSIBLY_WINNABLE}. It is sound but not complete - a definite verdict is always correct, and
 * {@code POSSIBLY_WINNABLE} asserts nothing.</li>
 * <li><strong>Full</strong> ({@link io.github.dlbbld.ashlarchess.unwinnability.UnwinnableFullAnalyzer}) - the paper's
 * Figure 9: deep search, three-valued: {@code WINNABLE}, {@code UNWINNABLE}, or {@code UNDETERMINED}. The direct
 * analysis record carries the concrete mate line witnessing a {@code WINNABLE} result. The undetermined case is bounded
 * by a 500&nbsp;000-position limit; most positions resolve well below it.</li>
 * </ul>
 *
 * <p>
 ...
 * positions; only a small, known set of unreachable constructions is mis-decided - for example a checkmate delivered by
 * an impossible double-bishop check over otherwise insufficient material (called unwinnable). Such positions cannot
 * occur in a legally played game; they are of interest only for puzzles or position composition, which is out of scope.
 *
 ...
 *
 * <p>
 * See {@code specification.md} section 3.2 for the design rationale and {@code fun22-spec.pdf} for the governing
