...
public final class BitboardLegalMoveFactory {
  ...
  public static LegalMove toLegalMove(BitboardPosition position, MoveSpecification moveSpec, Side movingSide) {
    ...
    if (moveSpec.castlingMove() != io.github.dlbbld.ashlarchess.board.enums.CastlingMove.NONE) {
   ...
   * {@link #emitCastlingLegalMoves} using {@link CastlingUtility}'s bitboard overloads. Returns an {@link List} sorted
   * by {@link LegalMove#compareTo}.
   */
  public static List<LegalMove> calculateLegalMoves(BitboardPosition bitboardPosition, Side sideToMove,
      CastlingRight castlingRight, long enPassantBit) {
    final Set<LegalMove> result = new TreeSet<>();
    calculateLegalMovesInto(result::add, bitboardPosition, sideToMove, castlingRight, enPassantBit);
   ...
   * rather than {@link LegalMove#compareTo} order. No {@link Set} / {@link TreeSet} / {@link List} is allocated by this
   * method - the search hot path passes a per-depth reusable buffer's {@code add} method as the sink. The order-sorting
   * set-based {@link #calculateLegalMoves} wraps this with a {@link TreeSet}-collecting sink so Board's public-API
  ...
  public static void calculateLegalMovesInto(Consumer<LegalMove> sink, BitboardPosition bitboardPosition,
      Side sideToMove, CastlingRight castlingRight, long enPassantBit) {
    bitboardPosition.legalMovesInto(spec -> sink.accept(toLegalMove(bitboardPosition, spec, sideToMove)), sideToMove,
        enPassantBit);
    emitCastlingLegalMoves(sink, bitboardPosition, sideToMove, castlingRight);
  }

  /**
   * Emits the legal castling moves for {@code sideToMove} (king-side and queen-side, in that order) to {@code sink},
   * using {@link CastlingUtility}'s bitboard-overload castling checks.
   */
  private static void emitCastlingLegalMoves(Consumer<LegalMove> sink, BitboardPosition bitboardPosition,
      Side sideToMove, CastlingRight castlingRight) {
    if (sideToMove == Side.WHITE) {
      if (CastlingUtility.calculateQueenSideCastlingCheck(bitboardPosition, sideToMove,
          castlingRight) == CastlingCheck.SUCCESS) {
        sink.accept(CastlingConstants.WHITE_QUEEN_SIDE_CASTLING_MOVE);
      }
      if (CastlingUtility.calculateKingSideCastlingCheck(bitboardPosition, sideToMove,
          castlingRight) == CastlingCheck.SUCCESS) {
      ...
      throw new IllegalArgumentException("emitCastlingLegalMoves requires Side.WHITE or Side.BLACK, got " + sideToMove);
    }
  }
}
